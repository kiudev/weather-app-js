export default function tomorrow(data) {
  ...
  const tomorrowData = data.list.find((item) => {
    ...
    return date.getHours() === tomorrow.getHours() && date.getDate() === tomorrow.getDate();
  });
...
<img src="http://openweathermap.org/img/wn/${
    tomorrowData.weather[0].icon
  }@2x.png" alt="${tomorrowData.weather[0].description}" />
  <div class="temp-range">
    <p class="temp">${parseInt(tomorrowData.main.temp)}°C</p>
    <div>
     <svg  xmlns="http://www.w3.org/2000/svg"  width="20"  height="20"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-arrow-up"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 5l0 14" /><path d="M18 11l-6 -6" /><path  d="M6 11l6 -6" /></svg>
      <p class="temp-max">${parseInt(tomorrowData.main.temp_max)} °C</p>
    </div>
    <div>
      ...
      <p class="temp-min">${parseInt(tomorrowData.main.temp_min)} °C</p>
    </div>
    <div>
    ...
    <p class="humidity">${tomorrowData.main.humidity}%</p>
    </div>
  </div>
</div>`;
}
